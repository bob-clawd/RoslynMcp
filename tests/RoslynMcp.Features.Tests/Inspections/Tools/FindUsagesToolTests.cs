...
{
    [Fact]
    public async Task FindUsagesAsync_WithWorkspaceRelativeDocumentPath_ReturnsRelativeReferences()
    {
        ...
        var sut = context.GetRequiredService<FindUsagesTool>();
        var symbol = await resolver.ExecuteAsync(CancellationToken.None, path: Path.Combine("ProjectCore", "Contracts.cs"), line: 31, column: 24);

        symbol.Error.ShouldBeNone();

        var result = await sut.ExecuteAsync(CancellationToken.None, symbol.Symbol!.SymbolId, scope: "document", path: Path.Combine("ProjectApp", "AppOrchestrator.cs"));
