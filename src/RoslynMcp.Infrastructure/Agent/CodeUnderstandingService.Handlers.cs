...
public sealed partial class CodeUnderstandingService
{
    private sealed class UnderstandCodebaseHandler
    {
        private readonly CodeUnderstandingService _owner;

        public UnderstandCodebaseHandler(CodeUnderstandingService owner)
        {
            _owner = owner;
        }
        ...
        public async Task<UnderstandCodebaseResult> HandleAsync(UnderstandCodebaseRequest request, CancellationToken ct)
        {
            ...
            var (solution, error) = await _owner.GetCurrentSolutionWithAutoBootstrapAsync(
            ...
            var metricResult = await _owner._analysisService.GetCodeMetricsAsync(new GetCodeMetricsRequest(), ct).ConfigureAwait(false);
            ...
            var hotspots = await _owner.BuildHotspotsAsync(solution, metricResult.Metrics, hotspotCount, ct).ConfigureAwait(false);
            return new UnderstandCodebaseResult(
                profile,
                modules,
                hotspots,
    ...
    private sealed class ExplainSymbolHandler
    {
        private readonly CodeUnderstandingService _owner;

        public ExplainSymbolHandler(CodeUnderstandingService owner)
        {
            _owner = owner;
        }
        ...
        public async Task<ExplainSymbolResult> HandleAsync(ExplainSymbolRequest request, CancellationToken ct)
        {
            ...
            var (_, bootstrapError) = await _owner.GetCurrentSolutionWithAutoBootstrapAsync(
            ...
            var symbolResult = await _owner.ResolveSymbolAtRequestAsync(request.SymbolId, request.Path, request.Line, request.Column, ct).ConfigureAwait(false);
            ...
            var signature = await _owner._navigationService.GetSignatureAsync(new GetSignatureRequest(symbolResult.Symbol.SymbolId), ct).ConfigureAwait(false);
            var outline = await _owner._navigationService.GetSymbolOutlineAsync(new GetSymbolOutlineRequest(symbolResult.Symbol.SymbolId, 1), ct).ConfigureAwait(false);
            var references = await _owner._navigationService.FindReferencesAsync(new FindReferencesRequest(symbolResult.Symbol.SymbolId), ct).ConfigureAwait(false);
    ...
    private sealed class ListTypesHandler
    {
        private readonly CodeUnderstandingService _owner;

        public ListTypesHandler(CodeUnderstandingService owner)
        {
            _owner = owner;
        }
        ...
        public async Task<ListTypesResult> HandleAsync(ListTypesRequest request, CancellationToken ct)
        {
            ...
            var (solution, solutionError) = await _owner.GetCurrentSolutionWithAutoBootstrapAsync(
    ...
    private sealed class ListMembersHandler
    {
        private readonly CodeUnderstandingService _owner;

        public ListMembersHandler(CodeUnderstandingService owner)
        {
            _owner = owner;
        }
        ...
        public async Task<ListMembersResult> HandleAsync(ListMembersRequest request, CancellationToken ct)
        {
            ...
            var typeSymbol = await _owner.ResolveTypeSymbolAsync(request, solution, ct).ConfigureAwait(false);
    ...
    private sealed class ResolveSymbolHandler
    {
        private readonly CodeUnderstandingService _owner;

        public ResolveSymbolHandler(CodeUnderstandingService owner)
        {
            _owner = owner;
        }
        ...
        public async Task<ResolveSymbolResult> HandleAsync(ResolveSymbolRequest request, CancellationToken ct)
        {
            ...
            {
                var symbol = await _owner._symbolLookupService.ResolveSymbolAsync(request.SymbolId!, solution, ct).ConfigureAwait(false);
                if (symbol == null)
                {
                    return new ResolveSymbolResult(
                        null,
            ...
            {
                var symbol = await _owner._symbolLookupService.GetSymbolAtPositionAsync(
                    solution,
                    request.Path!,
                    request.Line.Value,
    ...
    private sealed class ListDependenciesHandler
    {
        private readonly CodeUnderstandingService _owner;

        public ListDependenciesHandler(CodeUnderstandingService owner)
        {
            _owner = owner;
        }
